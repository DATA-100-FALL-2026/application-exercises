---
title: "AE 07 - Brexit - Telling Different Stories with Data"
author: "Your Name"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
1. Navigate to the `ae-07-brexit` folder in YOUR fork
2. Open `ae-07-brexit.Rmd`
...
```{r original-plot, fig.width=10, fig.height=4, fig.alt="Faceted bar chart of Brexit opinion counts by region, using the same x-axis scale across all facets."}
...
```{r free-scales, eval=FALSE, fig.width=10, fig.height=4, fig.alt="Faceted bar chart of Brexit opinion counts by region, with each facet using its own independent x-axis scale."}
...
```{r plot-proportions, eval=FALSE, fig.width=10, fig.height=4, fig.alt="Faceted bar chart of Brexit opinion proportions by region, with x-axis labeled as percentages."}
# Remember to change eval=FALSE to eval=TRUE!
...
```{r dodged-bars, eval=FALSE, fig.width=8, fig.height=5, fig.alt="Grouped bar chart with regions on the x-axis and side-by-side bars showing opinion proportions within each region."}
# Remember to change eval=FALSE to eval=TRUE!
...
3. Click **Commit**
4. Write message: "Completed AE 07 - Brexit"
...
3. Find `ae-07-brexit.pdf` in Files pane
