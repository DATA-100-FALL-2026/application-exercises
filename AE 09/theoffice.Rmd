---
title: "AE 09 - The Office - Predicting IMDB Ratings"
author: "Your Name"
date: "`r Sys.Date()`"
output: 
  html_document:
...
1. Navigate to the `ae-09-office` folder in YOUR fork
2. Open `ae-09-office.Rmd`
...
**A note on the `michael` indicator:** `season <= 7` is a simplification, not an exact rule — Michael Scott actually leaves partway through season 7 (not at a season boundary) and briefly reappears in the season 9 finale. As you build this variable, think about what this approximation gets right, what it might get wrong, and whether a more precise version would change your results later on.
...
**Task 3.3:** Revisit the note above about `season <= 7` as a stand-in for Michael's presence. Name one specific episode or situation where this indicator would be wrong, and briefly explain why.
...
**Hint for interpreting your coefficients:** In the `tidy()` output, the `estimate` column's *sign* tells you direction — positive means the rating goes up as the predictor increases (or the group is coded 1), negative means it goes down. The *size* of the estimate tells you how much, in the original units of `imdb_rating` (points on the rating scale). Read the sign first, then the magnitude.
...
**Why this matters:** A single train/test split gives you one RMSE estimate, and that estimate depends partly on which episodes happened to land in your test set. Cross-validation refits your model across 5 different folds and averages the results, giving you a more stable estimate of how well your model generalizes — and a sense of how much that estimate might vary. Keep this in mind for Exercise 10, where you'll compare a single test-set RMSE across two models.
...
**Reminder — what is the "Old Model"?** It's the model from the cross-validation lesson, and it uses a different (simpler) set of features than the one you just built: `season`, `episode`, `total_votes`, and `air_date` (converted into a month indicator), with no dialogue-based features at all — no `michael`, `jim_pct`, or holiday indicators. As you compare the two models below, keep that feature difference in mind: it's the main thing that could explain why one outperforms the other.
...
3. Click **Commit**
4. Write message: "Completed AE 09 - The Office"
...
3. Find `ae-09-office.pdf` in Files pane
